    ...
    (function() {
      ...
      Requestr.parsePage = function(request) {
        ...
        if (Requestr.service) {
          ...
          var i, length, currEl, requestUrls = [], originalRequests = [],
              callback, baseEl, elementToRemove,
          ...
          for (i = 0, length = requestingElements.length; (currEl = requestingElements[i]); i++) {
            ...
            if (Requestr.urlParsingEnabledElement[currEl.nodeName]) {
              ...
              // Building list of elements to resolve when results are returned.
              originalRequests.push({
                resolvedValue: currEl.src || currEl.href,
                originalValue: currEl.getAttribute('src') || currEl.getAttribute('href'),
                attribute: (currEl.src ? 'src' : (currEl.href ? 'href' : null))
              });
            }
          }
          /*
      ...
      Requestr.handleResponseResources = function(request) {
        ...
        var dataSet = request.target.response,
            docString = Requestr.documentString, i, j, currMatchEl,
        ...
        if (sourceSet) {
          for (i = 0; (currSource = sourceSet[i]); i++) {
            ...
            var url, urlBlob = Requestr.dataURItoBlob(currSource.Data, currSource.ContentType);
      ...
      if (Requestr.serialization && (Requestr.serialization.textContent || Requestr.serialization.innerText)) {
        Requestr.serialization = JSON.parse((Requestr.serialization.textContent || Requestr.serialization.innerText));
        // Getting the location of the dataURI service.
        if (Requestr.serialization && Requestr.serialization.service) {
          // TODO (jam@): Add the capability of doing multiple services (per domain).
          Requestr.service = Requestr.serialization.service;
        } else {
          // TODO (jam@): Add error handling for no service specificed.
        }
        // Checking for page to load or self.
        if (Requestr.serialization && Requestr.serialization.self) {
          // Loading self.
          Requestr.loadPage(window.location.href);
        } else if (Requestr.serialization && Requestr.serialization.owner) {
          // Loading external page.
          Requestr.loadPage(Requestr.serialization.owner);
        } else {
          // TODO (jam@): This means they want to load iframes or fragments.
        }
      } else {
        // TODO (jam@): Determine functionality if no serialization present
        // since there is no service defined.
      }
    }());
  }
}
